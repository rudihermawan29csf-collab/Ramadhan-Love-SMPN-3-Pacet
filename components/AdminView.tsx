...
const AdminView: React.FC<AdminViewProps> = ({ onLogout }) => {
  ...
  const handleEditBroadcast = (broadcast: Broadcast) => {
      setBroadcastInput(broadcast.message);
  ...
  const handleSaveBroadcast = () => {
      ...
      const broadcastData: Broadcast = {
          id: editingBroadcastId || `bc_${Date.now()}`,
          message: broadcastInput,
          createdAt: editingBroadcastId ? broadcasts.find(b => b.id === editingBroadcastId)?.createdAt || new Date().toISOString() : new Date().toISOString(),
          active: true
      };

      StorageService.saveBroadcast(broadcastData);
      setBroadcastInput('');
      setEditingBroadcastId(null);
      refreshData();
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 rounded-3xl shadow-sm flex flex-col">
          <h3 className="font-bold text-gray-700 mb-4 flex items-center justify-between">
            <div className="flex items-center"><Bell size={20} className="mr-2 text-pink-500" /> Broadcast</div>
            {editingBroadcastId && (
                <button onClick={() => { setEditingBroadcastId(null); setBroadcastInput(''); }} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar mb-4">
             {broadcasts.map(b => (
               <div key={b.id} className={`p-3 rounded-xl shadow-sm hover:bg-white/60 transition border ${editingBroadcastId === b.id ? 'bg-indigo-50 border-indigo-200 ring-1 ring-indigo-200' : 'bg-white/40 border-white'}`}>
          ...
          <div className="mt-auto">
            <input 
                ...
                className="w-full border border-gray-200 bg-white/50 rounded-xl px-4 py-3 text-sm mb-2 focus:ring-2 focus:ring-indigo-400 focus:outline-none placeholder-gray-400" 
            ...
            <button 
             ...
             >
               {editingBroadcastId ? 'Update Pesan' : 'Kirim Pesan'}
             </button>
          </div>
        </div>
      </div>
